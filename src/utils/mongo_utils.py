...
from src.db_models.tumatura import Users, UserRole, Roles
...
class MongoUtils:
    ...
    @staticmethod
    def get_user_data(*, customer_id: int = None, discord_name: str, email: str = None) -> Users:
        user = None
        if customer_id:
    ...
    def upsert_user_role(self, role_id: List[int], discord_name: str, *,
                         customer_id: int = None, email: str = None, expiration_date: datetime = None) -> None:
        user_data = self.get_user_data(customer_id=customer_id, discord_name=discord_name,
                                       email=email)
        ...
        for role in role_id:
            role_record = UserRole(role_id=role, exp_date=expiration_date if expiration_date else (
                    datetime.now() + timedelta(weeks=1)))
            user_data.roles.append(role_record)

        user_data.save()

    def upsert_user_data(self, discord_name: str, customer_id: int = None, email: str = None) -> None:
        user_data = self.get_user_data(customer_id=customer_id, discord_name=discord_name,
                                       email=email)
    ...
    def clean_expired_user_roles(self):
